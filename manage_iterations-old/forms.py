...
class IterationForm(forms.ModelForm):
    ...
    def __init__(self, org_info, *args, **kwargs):
        ...
        self.fields['Product'] = forms.ModelChoiceField(empty_label="Please select product",queryset=AR_product.objects.filter(ORG_ID__in=Subquery(org_info.values("id"))),widget=forms.Select(attrs={"class": "form-control"}))
        self.fields['Backlog'] = forms.ModelChoiceField(empty_label="Please select backlog",queryset=AR_BACKLOG.objects.none(),widget=forms.Select(attrs={"class": "form-control"}))

        if 'Product' in self.data:
            try:
                Product_id = int(self.data.get('Product'))
                self.fields['Backlog'].queryset = AR_BACKLOG.objects.filter(product_parent_id=Product_id)
            except (ValueError, TypeError):
                pass  # invalid input from the client; ignore and fallback to empty City queryset
        elif self.instance.pk:
            self.fields['Backlog'].queryset = AR_BACKLOG.objects.filter(product_parent=self.instance.Product)


        self.fields['Team'] = forms.ModelChoiceField(empty_label="Please select team",queryset=AR_team.objects.filter(ORG_id__in=Subquery(org_info.values("id"))),widget=forms.Select(attrs={"class": "form-control"}))
