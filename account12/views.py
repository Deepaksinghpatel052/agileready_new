...
def test_mail(request):
    ...
    data_content = {"BASE_URL": settings.BASE_URL, "yourname": "deepak patel", "user_email": user_email,
                    "password": password,
...
def account_activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        # org_id_set = get_user_info.org_id
        # if org_id_set == 0:
        #     org_id = AR_organization.objects.get(created_by=user)
        #     org_id_set = org_id.id

        request.session['user_id'] = get_user_info.id
        # org_id_k = AR_organization.objects.get(id=org_ins)
    ...
    if request.method == "POST":
        ...
        if User.objects.filter(email=user_email).exists():
            ...
            if Ar_user.objects.filter(phone=number).exists():
                ...
                if AR_organization.objects.filter(organization_url=organization_url).filter(
                        ~Q(organization_url="")).exists():
                    ...
                    if AR_organization.objects.filter(organization_name=organization).exists():
                        ...
                        ar_org = AR_organization(org_id=org_id, organization_name=organization,
                                                 organization_url=organization_url, created_by=user, update_by=user)
                        ar_org.save()
                    org_ins = get_object_or_404(AR_organization, organization_name=organization)

                    Ar_users = Ar_user(user_id=user.id, user_name=yourname, country=country, city=city, state=state,
                                       zip=zip_code, phone=number, org_id=org_ins)
                    Ar_users.save()
                    ...
                    logo_image = settings.BASE_URL + 'static/img/basic/logo.png'
                    data_content = {"BASE_URL": settings.BASE_URL, "yourname": yourname, "user_email": user_email,
                                    "password": password, "logo_image": logo_image,
                                    "varification_link": varification_link}
                    email_content = render_to_string('email_template/email_send_for_create_new_account.html',
                                                     data_content)
                    msg = email.message.Message()
                    msg['Subject'] = 'Account Create successfully'
                    msg['From'] = settings.EMAIL_HOST_USER
                    ...
                    return JsonResponse({"status": "1",
                                         "message": "Registration is done successfully, verification link is sent to your email!"})
...
def send_varification_link(request):
    if request.method == "POST":
        ...
        if User.objects.filter(email=user_email).exists():
            ...
            ################################################ EMAL SEND CODE START ##############
            logo_image = settings.BASE_URL + 'static/img/basic/logo.png'
            data_content = {"BASE_URL": settings.BASE_URL, "logo_image": logo_image, "user_name": user_info.user_name,
                            "varification_link": varification_link}
