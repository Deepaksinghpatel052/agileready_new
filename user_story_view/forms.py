...
class Ar_User_Story_Form(forms.ModelForm):
    ...
    user_story_status = forms.ModelChoiceField(empty_label="Please select user story status or None", queryset=AR_US_STATUS.objects.all(),widget=forms.Select(attrs={"class": "form-control"}))
    class Meta:
        model = AR_USER_STORY
    ...
    def __init__(self, user_id,org_info, *args, **kwargs):
        ...
        # self.fields['owner'] = forms.ModelChoiceField(required=False,empty_label="Please select team member owner - Assigned to or None",queryset=Ar_user.objects.filter(~Q(user_name="")).filter(org_id=org_info),widget=forms.Select(attrs={"class": "form-control", 'name': 'owner'}))
